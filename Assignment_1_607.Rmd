...
Calling required packages in r
```{r include=FALSE}
library(tidyverse)
library(dplyr)
```
...
##View Data
View data for ads that had danger and weren't intended to be funny:

```{r include=FALSE}
view(Superbowl_ads_not_funny_with_danger)
```
...
##View Data
View data for ads that had a celebrity and was patriotic:

```{r include=FALSE}
view(Superbowl_ads_patriotic_celebrity)
```
...
##View Data
View data for ads that were supposed to be funny with animal and sexual content

```{r include=FALSE}
view(Superbowl_ads_funny_animal_sexual)
```
